---
layout: post
title: "New Storage Pool and Installing a New VM in Text Mode in KVM - HowTo [DRAFT]"
...
```
$ ssh dusko@kvmhost2.chem.ubc.ca
```
...
Name:           default
UUID:           a721f571-199d-4cd5-9384-d4364f756823
State:          running
Persistent:     yes
Autostart:      yes
Capacity:       4.55 TiB
...
Name:           tmp
UUID:           5a272ff5-452e-4aee-ad8a-24527d41c13d
State:          running
Persistent:     yes
Autostart:      yes
Capacity:       113.98 GiB
...
```
$ vi ~/vol-template-for_chemapps2.xml
```

```
$ cat ~/vol-template-for_chemapps2.xml
<volume type='file'>
  <name>chemapps2.qcow2</name>
  <key>/iscsisan/images/chemapps2.qcow2</key>
  <source>
  </source>
  <capacity unit='G'>240</capacity>
  <target>
    <path>/iscsisan/images/chemapps2.qcow2</path>
    <format type='qcow2'/>
    <permissions>
      <mode>0644</mode>
      <owner>107</owner>
...
$ sudo \
 virsh vol-create \
 --pool default \
 ~/vol-template-for_chemapps2.xml
```

Output:

```
Vol chemapps2.qcow2 created from /home/dusko/vol-template-for_chemapps2.xml
```
...
$ sudo virsh vol-list --pool default
 Name                 Path
------------------------------------------------------------------------------
 chemapps2.qcow2      /iscsisan/images/chemapps2.qcow2
 exceed.qcow2         /iscsisan/images/exceed.qcow2
 fbsdtest1.qcow2      /iscsisan/images/fbsdtest1.qcow2
 groups.qcow2         /iscsisan/images/groups.qcow2
 mail.qcow2           /iscsisan/images/mail.qcow2
...
$ ls -lh /iscsisan/images/chemapps2.qcow2
-rw-r--r--. 1 qemu qemu 196K Jun 21 13:44 /iscsisan/images/chemapps2.qcow2

$ date
Fri Jun 21 13:44:30 PDT 2024

$ file /iscsisan/images/chemapps2.qcow2
/iscsisan/images/chemapps2.qcow2: QEMU QCOW Image (v2), 257698037760 bytes

$ qemu-img info /iscsisan/images/chemapps2.qcow2
image: /iscsisan/images/chemapps2.qcow2
file format: qcow2
...
$ sudo virsh vol-info /iscsisan/images/chemapps2.qcow2
Name:           chemapps2.qcow2
Type:           file
Capacity:       240.00 GiB
...
```
$ sudo virsh vol-dumpxml /iscsisan/images/chemapps2.qcow2 --pool default
<volume type='file'>
  <name>chemapps2.qcow2</name>
  <key>/iscsisan/images/chemapps2.qcow2</key>
  <source>
...
$ ls -Alh /iscsisan/images/
total 724G
-rw-r--r--. 1 qemu qemu 196K Jun 21 13:44 chemapps2.qcow2
-rw-------. 1 qemu qemu  38G Jun 15 08:01 exceed.qcow2
-rw-r--r--. 1 root root 2.0G Dec  6  2019 fbsdtest1.qcow2
...
```
$ sudo mkdir /iscsisan/ISO
```


```
$ ssh -t dusko@kvmhost1.chem.ubc.ca 'ls /iscsiadd02/images'
rhel-9.2-x86_64-dvd.iso
Connection to kvmhost1.chem.ubc.ca closed.
```

```
$ touch /iscsisan/ISO/testfile
touch: cannot touch ‘/iscsisan/ISO/testfile’: Permission denied

$ printf %s\\n Testing | sudo tee -a /iscsisan/ISO/testfile
Testing

$ rm -i /iscsisan/ISO/testfile
rm: remove write-protected regular file ‘/iscsisan/ISO/testfile’? y
rm: cannot remove ‘/iscsisan/ISO/testfile’: Permission denied

$ sudo rm -i /iscsisan/ISO/testfile
rm: remove regular file ‘/iscsisan/ISO/testfile’? y
```


This `scp` took around 1.5 minutes:

```
$ scp \
 dusko@kvmhost1.chem.ubc.ca:/iscsiadd02/images/rhel-9.2-x86_64-dvd.iso \
 /tmp/
dusko@kvmhost1.chem.ubc.ca's password:

rhel-9.2-x86_64-dvd.iso                       100% 9151MB 101.3MB/s   01:30
```

```
$ sudo cp -i /tmp/rhel-9.2-x86_64-dvd.iso /iscsisan/ISO/

$ ls -lh /iscsisan/ISO/rhel-9.2-x86_64-dvd.iso
-rw-r--r--. 1 root root 9.0G Jun 21 14:05 /iscsisan/ISO/rhel-9.2-x86_64-dvd.iso

$ rm -i /tmp/rhel-9.2-x86_64-dvd.iso
rm: remove regular file ‘/tmp/rhel-9.2-x86_64-dvd.iso’? y
```


NOTE for the following `virt-install` command: Instead of the more common `--disk=<disk_size>`, in this case you will use `--disk vol=poolname/volname` option for specifying storage volume.
...
$ sudo \
 ...
 --memory 8192 \
 --vcpus 4 \
 --disk vol=default/chemapps2.qcow2 \
 --os-variant rhel9.2 \
 --graphics none \
...
```
Jun 21 15:55:36 chemapps2 systemd[1]: Starting dnf makecache...
Jun 21 15:55:37 chemapps2 dnf[1738]: Updating Subscription Management repositories.
Jun 21 15:55:37 chemapps2 dnf[1738]: Unable to read consumer identity
Jun 21 15:55:37 chemapps2 dnf[1738]: This system is not registered with an entitlement server. You can use subscription-manager to register.
Jun 21 15:55:37 chemapps2 dnf[1738]: Failed determining last makecache time.
Jun 21 15:55:37 chemapps2 dnf[1738]: There are no enabled repositories in "/etc/yum.repos.d", "/etc/yum/repos.d", "/etc/distro.repos.d".
Jun 21 15:55:37 chemapps2 systemd[1]: dnf-makecache.service: Deactivated successfully.
Jun 21 15:55:37 chemapps2 systemd[1]: Finished dnf makecache.
Jun 21 16:25:36 chemapps2 systemd[1]:
```
...
The system has been registered with ID: 9afd7f11-0e67-4aa9-af85-0231b40c061d
The registered system name is: kvmhost2.chem.ubc.ca
...
system identity: 9afd7f11-0e67-4aa9-af85-0231b40c061d
name: kvmhost2.chem.ubc.ca
org name: UBC IT Services
org ID: UBCITServices
...
Repo URL:  https://satellite6.it.ubc.ca/pulp/content/UBCITServices/Library/RHEL-
...
SKU:                 RH01157
Contract:            12196201
Pool ID:             0ee7bd828ee3666e018f5dfeabab1638
...
[rhsm]
   ...
   baseurl = https://satellite6.it.ubc.ca/pulp/content/
...
```
$ sudo yum list installed | grep -i katello
katello-ca-consumer-satellite6.it.ubc.ca.noarch
...
Unregistering from: satellite6.it.ubc.ca:443/rhsm
...
```
$ sudo subscription-manager register --org=UBCITServices --activationkey=RHEL7
```

Output:

```
The system has been registered with ID: 8f326fad-c7a4-4df1-ad54-f60976358b11
The registered system name is: kvmhost2.chem.ubc.ca
Installed Product Current Status:
Product Name: Red Hat Enterprise Linux Server
Status:       Not Subscribed
...
```
The system has been registered with ID: a4bba9cb-23ae-4117-9ef9-b42198800530
The registered system name is: kvmhost2.chem.ubc.ca
Installed Product Current Status:
Product Name: Red Hat Enterprise Linux Server
Status:       Not Subscribed
...
NTP servers:
ntp1.netcom.ubc.ca (status: working)
ntp2.netcom.ubc.ca (status: working)
hub.ubc.ca (status: working)
dns3.ubc.ca (status: working)
ns-int-1.netcom.ubc.ca (status: working)
ns-int-2.netcom.ubc.ca (status: working)
...
Add two NTP servers (`ntp.ubc.ca`, `ntp2.ubc.ca`).
...
ns-int-1.netcom.ubc.ca (status: working)
ns-int-2.netcom.ubc.ca (status: working)
ntp.ubc.ca (status: working)
ntp2.ubc.ca (status: working)
...
Wired (ens3) connected
 IPv4 Address: 137.82.59.176 Netmask: 255.255.255.0 Gateway: 137.82.59.254
 DNS: 137.82.1.2,142.103.1.42

Host Name:
  Current host name: dhcp-main-176.chem.ubc.ca

1) Set host name
2) Configure device ens3

1) Host Name:
   Host Name: chemapps2.chem.ubc.ca
...
Creating swap on /dev/mapper/rhel_dhcp--main--176-swap
Creating xfs on /dev/mapper/rhel_dhcp--main--176-home
Creating xfs on /dev/mapper/rhel_dhcp--main--176-root
...
...
* [Red Hat Satellite 6 (RHS6) - How To - Client Registration (Requires CWL login)](https://confluence.it.ubc.ca/display/ISSP/RHS6+-+How+To+-+Client+Registration)

* [Red Hat Satellite 6 (RHS6) - Troubleshooting](https://confluence.it.ubc.ca/display/ISSP/RHS6+-+Troubleshooting)
