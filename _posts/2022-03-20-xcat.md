...
that is, configuration of this example) scripts (both `postscripts` and 
`postbootscripts`) to the compute node that is being provisioned:
...
The first terminal **might**  (it doesn't always show this message) 
...
command here: `rpower abacus101 boot`.   
...
power back on; that is, it wouldn't actually restart.   

Therefore, I first power the node off and then power it on so run this 
in the second terminal:

```
# rpower abacus109 off 
```

The first terminal updates with the following:

```
[abacus109] 19:37:36 Node status is changed to powering-off
```

Now, from the second terminal, power on the compute node:
...
```
[abacus109] 19:37:48 Node status is changed to powering-on
```
