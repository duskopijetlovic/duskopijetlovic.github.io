...
node that you'll give to this BMC (`xbcmm2`), while the 
...
timeout. This flag **must** be used with `-m` flag.

`-t timeout`
Specify the timeout, in minutes, to wait for the expectedstatus
specified by `-m` flag. This is a **required** flag if the `-m` flag 
is specified.   
Power off, then on.
